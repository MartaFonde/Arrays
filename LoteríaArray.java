...
public class Ejer9_Bol5{
    ...
    /**
     * Pide seis enteros al usuario, separados por comas, y los guarda (sin comas) en una colección. 
     * Si el usuario introduce menos de 6 números o no introduce comas, da error 
     * y tiene que volver a introducir datos.
    ...
    /**
     * Comprueba si los números de la colección están en el rango (1, 49).
     * Si da error, el usuario tendrá que volver a introducir 
    ...
    public static void comprobacionNum(ArrayList<Integer> colecUsuario){
        ...
        for(int i=0; i<colecUsuario.size(); i++){
            while(colecUsuario.get(i)<1 || colecUsuario.get(i)>49 ||
                    valoresRepetidos(i, colecUsuario)){
                System.out.printf("Error. Número incorrecto %d.\nIntroduce un número entre 1 y 49: ", colecUsuario.get(i));                
    ...
    /**
     * Comprueba si el valor de la colección en una posición está contenido en la colección.
    ...
    public static void datosCpu(ArrayList<Integer> colecCpu){
        for(int i=0; i<6; i++){
            colecCpu.add((int)(Math.random()*49+1));
    ...
    public static void mostrarDatos(ArrayList<Integer> colec){
        for(int i=0; i<colec.size(); i++){
            if(i<colec.size()-1){
                System.out.printf("%d,", colec.get(i));
            }else{
                System.out.printf("%d\n", colec.get(i));
            }
        }
